...
const npwd   = (global.exports as any)['npwd'];

// ── ATM locations (GTA5 world coords) ─────────────────────────────────────────
// Update coords to match actual ATM prop positions on your server.

const ATM_COORDS = [
  { id: 1,  name: 'Legion Square',     area: 'Downtown LS',    x: -1051.9, y: -774.4,   z: 17.0  },
  ...
  { id: 7,  name: 'LSIA Terminal',     area: 'Airport',        x: -1034.5, y: -2726.0,  z: 20.2  },
...
on('__cfx_nui:getBalance', (_data: unknown, cb: Function) => {
  QBCore.Functions.TriggerCallback('bankingapp:getBalance', (resp: unknown) => cb(resp));
});

RegisterNuiCallbackType('getTransactions');
on('__cfx_nui:getTransactions', (_data: unknown, cb: Function) => {
  QBCore.Functions.TriggerCallback('bankingapp:getTransactions', (resp: unknown) => cb(resp));
});

RegisterNuiCallbackType('getPlayerByPhone');
on('__cfx_nui:getPlayerByPhone', (data: { phoneNumber: string }, cb: Function) => {
  QBCore.Functions.TriggerCallback('bankingapp:getPlayerByPhone', (resp: unknown) => cb(resp), data.phoneNumber);
});

RegisterNuiCallbackType('transfer');
on('__cfx_nui:transfer', (data: { phoneNumber: string; amount: number }, cb: Function) => {
  QBCore.Functions.TriggerCallback('bankingapp:transfer', (resp: unknown) => cb(resp), data);
});

RegisterNuiCallbackType('getAtmLocations');
on('__cfx_nui:getAtmLocations', (_data: unknown, cb: Function) => {
  const ped = PlayerPedId();
  const coords = GetEntityCoords(ped, false);
  const [px, py, pz] = [coords[0], coords[1], coords[2]];

  const locations = ATM_COORDS
    .map((atm) => {
      const dx = atm.x - px;
      const dy = atm.y - py;
      const dz = atm.z - pz;
...
onNet('bankingapp:client:incomingTransfer', (data: { amount: number; from: string }) => {
